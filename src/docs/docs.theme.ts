...
export const theme: ThemeInterface = {
  ...
  schema: {
    ...
    typeTitleColor: (_theme) => _theme.schema.typeNameColor,
    requireLabelColor: (_theme) => _theme.colors.error.main,
    labelsTextSize: '0.9em',
    nestingSpacing: '1em',
    nestedBackground: '#fafafa',
    arrow: {
      size: '1.1em',
      color: (_theme) => _theme.colors.text.secondary,
    },
  },
  typography: {
    fontSize: '14px',
  ...
  sidebar: {
    ...
    textColor: '#333333',
    activeTextColor: (_theme) =>
      _theme.sidebar.textColor !== _theme.sidebar!.textColor
        ? _theme.sidebar.textColor
        : _theme.colors.primary.main,
    groupItems: {
    ...
    arrow: {
      size: '1.5em',
      color: (_theme) => _theme.sidebar.textColor,
    },
  },
  logo: {
...
export function resolveTheme(_theme: ThemeInterface): ResolvedThemeInterface {
  const resolvedValues = {};

  let counter = 0;

  const setProxy = (obj: any, path: string): void => {
        ...
        Object.defineProperty(obj, k, {
          get() {
            if (!resolvedValues[currentPath]) {
              ...
              resolvedValues[currentPath] = val(_theme);
            }
  ...
  setProxy(_theme, '');
  return JSON.parse(JSON.stringify(_theme));
}
